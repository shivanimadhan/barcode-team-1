...
def calculate_mean_correlation_length(radial_avg_lst: np.ndarray, micron_pixel_ratio: float, binning_factor: float) -> float:
...
def analyze_optical_flow(video: np.ndarray, name: str, flow_config: OpticalFlowConfig, in_config: ReaderConfig, out_config: WriterConfig) -> FlowResults:
    ...
    for frame_pair in flow_field_indices:
        ...
        velocity_correlations.append(v_rad_avg[:correlation_max])
    ...
    correlation_rad_avg_lst = np.array(velocity_correlations)
    
    correlation_length, g_r_list = calculate_mean_correlation_length(correlation_rad_avg_lst, um_pix_ratio, downsample)
    ...
    results = FlowResults(mean_speed = mean_speed, delta_speed = delta_speed, mean_theta = theta, mean_sigma_theta = sigma_theta, velocity_correlation_length = correlation_length)
    return results
